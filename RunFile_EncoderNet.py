...
filename = "./Models_EncoderNet/ValidationLoss.csv"
...
learning_rate = 0.001
enoder_loss_hyperparameter = 1e-2
...
def validation_loop():
    ...
    for sample_batched in dataloader_validation:
        ...
        out1, out2 = model(sample_batched['image'].cuda())
        loss = criterion2(out1, sample_batched['lesions'].cuda()) + enoder_loss_hyperparameter*criterion2(out2, sample_batched['image'].cuda())
        count+=1
...
#         out1, out2 = model(sample_batched['image'].cuda())
        ...
        loss = criterion2(out1, sample_batched['lesions'].cuda()) + enoder_loss_hyperparameter*criterion2(out2, sample_batched['image'].cuda())
#        print(enoder_loss_hyperparameter*criterion2(out2, sample_batched['image'].cuda()).item())
...
#     loss.cpu().detach()
    ...
    model.cpu()
    torch.save(model.state_dict(), './Models_EncoderNet/panet_model_'+str(epoch))
    model.cuda()
    scheduler.step()
